  ...
  <div class="col-md-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        ...
        @endif
        <form action="{{ route('payroll-component.update', $component->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
          <div class="mb-3">
            <label for="exampleInputNumber1" class="form-label">Nama Komponen</label>
            <input type="text" class="form-control" name="nama_komponen" value="{{$component->nama_komponen}}" require>
          </div>

          <div class="mb-3">
            <label for="exampleInputNumber1" class="form-label">Jenis Komponen</label>
            <select name="jenis" class="form-control" id="">
                <option value="Allowance" {{ $component->jenis == 'Allowance' ? 'selected' : '' }}>Allowance</option>
                <option value="Deduction" {{ $component->jenis == 'Deduction' ? 'selected' : '' }}>Deduction</option>
            </select>
          </div>
